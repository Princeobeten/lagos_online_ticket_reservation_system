 ...
 * Matches on the booking reference alone, so it works for a seat that was only
 * held as well as one that was already paid for and is now being cancelled.
 */
export async function releaseSeats({ tripId, bookingRef }) {
  await Trip.updateOne({ _id: tripId }, { $pull: { seats: { bookingRef } } });
}
