...
/** The signed-in user, but only if they own the platform. */
...
/**
 * Guard for the admin API. Passengers get 403 rather than 401, so the client
 * knows they are signed in but not allowed here.
 */
export async function requireAdmin() {
  ...
  if (user.role !== "admin") {
    const error = new Error("This area is for platform administrators only.");
    error.status = 403;
    throw error;
  }
  return user;
